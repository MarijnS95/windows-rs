...
/// Uses [`widestring::UCStr`] for null-terminated, checked strings.
pub struct PWSTR(pub *mut widestring::WideChar);
impl PWSTR {
    ...
    fn as_wide(&self) -> &widestring::WideCStr {
        if self.0.is_null() {
            Default::default()
        } else {
            unsafe { widestring::WideCStr::from_ptr_str(self.0) }
        }
    }
}
...
impl ::std::cmp::PartialEq<&str> for PWSTR {
    fn eq(&self, other: &&str) -> bool {
        let other = unsafe { widestring::WideCString::from_str_unchecked(other) };
...
unsafe impl ::windows::Abi for PWSTR {
    ...
    fn drop_param(param: &mut ::windows::Param<Self>) {
        if let ::windows::Param::Boxed(value) = param {
            if !value.0.is_null() {
                unsafe { widestring::WideCString::from_raw(value.0) };
            }
        }
    }
}
impl<'a> ::windows::IntoParam<'a, PWSTR> for &'a str {
    fn into_param(self) -> ::windows::Param<'a, PWSTR> {
        ::windows::Param::Boxed(PWSTR(
            widestring::WideCString::from_str(self).unwrap().into_raw(),
        ))
    }
}
