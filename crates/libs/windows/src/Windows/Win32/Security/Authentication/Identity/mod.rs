...
where
    P2: windows_core::Param<windows_core::PCWSTR>,
{
    windows_targets::link!("sspicli.dll" "system" fn SecAllocateAndSetCallTarget(lpipaddress : *const u8, cchipaddress : u32, targetname : windows_core::PCWSTR, freecallcontext : *mut i32) -> windows_core::HRESULT);
    unsafe {
        let mut result__ = core::mem::zeroed();
        SecAllocateAndSetCallTarget(core::mem::transmute(lpipaddress.as_deref().map_or(core::ptr::null(), |slice| slice.as_ptr())), lpipaddress.as_deref().map_or(0, |slice| slice.len().try_into().unwrap()), targetname.param().abi(), &mut result__).map(|| core::mem::transmute(result__))
    }
}
#[inline]
pub unsafe fn SecAllocateAndSetIPAddress(lpipaddress: &[u8]) -> windows_core::Result<i32> {
    windows_targets::link!("sspicli.dll" "system" fn SecAllocateAndSetIPAddress(lpipaddress : *const u8, cchipaddress : u32, freecallcontext : *mut i32) -> windows_core::HRESULT);
    unsafe {
        let mut result__ = core::mem::zeroed();
        SecAllocateAndSetIPAddress(core::mem::transmute(lpipaddress.as_ptr()), lpipaddress.len().try_into().unwrap(), &mut result__).map(|| core::mem::transmute(result__))
    }
}
#[inline]
...
impl Default for KERB_CHANGEMACHINEPASSWORD_REQUEST {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
...
impl Default for NETLOGON_TARGET_INFO {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
...
impl Default for SecPkgCredentials_KdcNetworkSettingsW {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
