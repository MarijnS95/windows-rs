...
pub struct PWSTR(pub *mut WCHAR);

impl PWSTR {
    /// Construct a new `PWSTR` from a raw pointer.
    pub const fn from_raw(ptr: *mut WCHAR) -> Self {
        Self(ptr)
    }
    ...
    pub unsafe fn as_wide(&self) -> &[WCHAR] {
        let len = dbg!(super::wcslen(PCWSTR::from_raw(self.0)));
